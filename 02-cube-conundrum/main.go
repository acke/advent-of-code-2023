...
func main() {
    ...
    for i, line := range lines {
         ...
         set := strings.ReplaceAll(result[1], ";", ",")
...
func GameValid (cubes []string) bool {
    for _, subset := range cubes {
        switch {
        case  strings.Contains(subset, "red"):
            if GetNumber(subset, "red") > 12 {
                return false
            }
        case  strings.Contains(subset, "green"):
            if GetNumber(subset, "green") > 13 {
                return false
            }
        case  strings.Contains(subset, "blue"):
            if GetNumber(subset, "blue") > 14 {
                return false
            }
        }
...
func GetNumber(subset, color string) int {
    cubes := strings.ReplaceAll(subset, color, "")
    cubes = strings.ReplaceAll(cubes, " ", "")

    number, err := strconv.Atoi(cubes)
